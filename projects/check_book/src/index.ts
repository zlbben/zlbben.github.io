...
async function startUp() {
  ...
  try {
    await Promise.all(
      books.map(async (val: { name: string; url: string[] }) => {
        const page = await browser.newPage();
        await page.goto(val.url[0], { timeout: 0 });
        let info = await page.$eval(piaotian.BIQUGE, (el: any) => ({
          text: el.innerText,
          href: el.href
        }));
        info.name = val.name;
  ...
  fs.writeFileSync(histryPath, JSON.stringify(updateHistry, undefined, 2), {
    encoding: 'utf8'
  });
